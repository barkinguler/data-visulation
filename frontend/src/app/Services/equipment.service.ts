...
export class EquipmentService {
  ...
  async createEquipment(equipmentInfo) {
    ...
    return await this.http.post(this.equipmentBaseUrl + 'createEquipment', equipmentInfo, { headers: this.header })
  ...
  findEquipment(id: number): Observable<any> {
    ...
    return this.http
      .get(
        this.equipmentLogApiBaseUrl + 'getLogByWorkerId/' + id, { headers: this.header }
      ).pipe(
        tap(responseData => {
          const postArray = [];
  ...
  async findEquipmentByWorkplaceId(workplaceId) {
    document.body.style.cursor = 'wait';
    this.equipments = this.http.get(this.equipmentBaseUrl + 'getAllEquipmentByWorkplaceId/' + workplaceId, { headers: this.header })
      .toPromise().catch(header => {
        this.messageService.catchHeader(header);
      });
    document.body.style.cursor = 'default';
  ...
  async updateEquipmentById(equipmentId, equipmentInfo) {
    document.body.style.cursor = 'wait';
    let response = this.http.put(this.equipmentBaseUrl + 'updateEquipmentById/' + equipmentId, equipmentInfo, { headers: this.header })
      .toPromise().catch(header => {
        this.messageService.catchHeader(header);
      });
    document.body.style.cursor = 'default';
  ...
  async deleteEquipmnetById(equipmentId) {
    document.body.style.cursor = 'wait';
    let response = this.http.delete(this.equipmentBaseUrl + 'deleteEquipmentById/' + equipmentId, { headers: this.header })
      .toPromise().catch(header => {
        this.messageService.catchHeader(header);
      });
    document.body.style.cursor = 'default';
