...
export class UpdateEquipmentComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.equipmentService.findEquipmentByWorkplaceId(this.employerWorkplaceId).then(res => {
      this.equipments = res;
    });
  }

  updateEquipmentById(equipment) {
    this.equipmentService.updateEquipmentById(equipment.id, equipment).then(res => {
