...
export class WorkerGraphicComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute,
    ...
    private workerService: WorkersService, private modalService: ModalService, private newgraphicService: NewgraphicService) { }
  ...
  getWorkerStatus() {
    ...
    this.workstatus_Subscription = this.workerService.findWorkStatus(this.id).subscribe(
      (value => {
        ...
        this.newgraphicService.workerStatus.next({ data: _.cloneDeep(this.workerStatusData), label: _.cloneDeep(this.workerLabel), type: 'status', id: this.id });
      }));
  }
  ...
  addFavority() {
    let graphic: Ifavories = { data: this.workerStatusData, label: this.workerLabel, type: 'status', id: this.id };
